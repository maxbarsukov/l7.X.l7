...
```
ОШИБКА: "Доллар, доллар, грязная зеленая бумажка!"
...
вывести ₽перем1 + ₽перем2; // 846.346808
```
...
// Короткий синтаксис
₽получ_привет_мир = фн(): строка => 'Привет, мир!';
```
...
выведи ₽a; // Вывод переменной ₽а

₽b = 'a';
выведи ₽₽b; // Вывод переменной ₽а (дополнительный ₽ перед переменной ₽b)

выведи ₽{'a'}; // Вывод переменной ₽a

имя_функции(); // Вызов функции имя_функции
₽c = 'имя_функции';
₽c(); //Вызов функции имя_функции

₽d = 'ИмяКласса';
₽obj = новый ИмяКласса; // Создание объекта класса ИмяКласса
₽obj = новый ₽d(); // Создание объекта класса ИмяКласса
...
класс МойКласс
{
    публичное константное КОНСТ_ЗНАЧЕНИЕ = 'Значение константы';
}
...
class MoyKlass
{
    public const KONST_ZNACHENIE = 'Значение константы';
}
...
<small align="right">* Использованный в описании курс доллара актуален на 06.05.2022 и не должен использоваться для биржевых торгов или финансовых сделок</small>
